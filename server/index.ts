...
async function main() {
  ...
  console.log(
    `[openjev] env: openai_key=${Boolean(process.env.OPENAI_API_KEY || process.env.CEREBRAS_API_KEY || process.env.GROQ_API_KEY)} ` +
      `decider_url=${process.env.DECIDER_BASE_URL || "http://localhost:8000"}`
  );
  ...
  app.get("/api/health", (_req, res) => {
    res.json({
      ok: true,
      service: "openjev",
      backends: ["parallel", "oneshot", "decider"],
      decider_url: process.env.DECIDER_BASE_URL || "http://localhost:8000",
    });
  });
  ...
  app.listen(PORT, () => {
    ...
    console.log(`  → modes: parallel | oneshot | decider (Mapika/decider)\n`);
  });
}
